...
import static se.bjurr.prnfb.admin.AdminFormValues.FIELDS.valueOf;
...
public class SettingsStorage {
 ...
 public static PrnfbButton getPrnfbButton(AdminFormValues adminFormValues) throws ValidationException {
  ...
  return new PrnfbButton(title, visibility, find(adminFormValues, predicate(FORM_IDENTIFIER.name())).get(VALUE));
 }
 ...
 public static PrnfbNotification getPrnfbNotification(AdminFormValues adminFormValues) throws ValidationException {
  ...
  if (tryFind(adminFormValues, predicate(proxy_server.name())).isPresent()) {
   prnfbNotificationBuilder.withProxyServer(find(adminFormValues, predicate(proxy_server.name())).get(VALUE));
  }
  if (tryFind(adminFormValues, predicate(proxy_port.name())).isPresent()) {
   prnfbNotificationBuilder.withProxyPort(find(adminFormValues, predicate(proxy_port.name())).get(VALUE));
  }
  if (tryFind(adminFormValues, predicate(proxy_user.name())).isPresent()) {
   prnfbNotificationBuilder.withProxyUser(find(adminFormValues, predicate(proxy_user.name())).get(VALUE));
  }
  if (tryFind(adminFormValues, predicate(proxy_password.name())).isPresent()) {
   prnfbNotificationBuilder.withProxyPassword(find(adminFormValues, predicate(proxy_password.name())).get(VALUE));
  }
  ...
  if (tryFind(adminFormValues, predicate(password.name())).isPresent()) {
   prnfbNotificationBuilder.withPassword(find(adminFormValues, predicate(password.name())).get(VALUE));
  }
  if (tryFind(adminFormValues, predicate(filter_string.name())).isPresent()) {
   prnfbNotificationBuilder.withFilterString(find(adminFormValues, predicate(filter_string.name())).get(VALUE));
  }
  if (tryFind(adminFormValues, predicate(filter_regexp.name())).isPresent()) {
   prnfbNotificationBuilder.withFilterRegexp(find(adminFormValues, predicate(filter_regexp.name())).get(VALUE));
  }
  if (tryFind(adminFormValues, predicate(method.name())).isPresent()) {
   prnfbNotificationBuilder.withMethod(find(adminFormValues, predicate(method.name())).get(VALUE));
  }
  if (tryFind(adminFormValues, predicate(post_content.name())).isPresent()) {
   prnfbNotificationBuilder.withPostContent(find(adminFormValues, predicate(post_content.name())).get(VALUE));
  }
